...
誤判成「已解決」。同名歧義的否決同樣以「歌手是否被佐證」為條件，而不是「有沒有給
歌手」：一個對不上任何候選的頻道名稱不具區辨力，行為必須與完全沒給歌手一致。
...
- **歧義否決改以「歌手是否被佐證」為條件**，並統一使用歌名分數比較。先前只在歌手為空時
  才啟用，導致一個對不上任何候選的頻道名稱會靜默選走排序第一的同名演出。
...
- **已儲存歌詞的管理介面**：搜尋、檢視實際內容、單筆刪除、批次刪除搜尋命中項目、
  總數顯示。
- **歧義否決改由長度決定**：先前只要另一個同名候選在 8 秒內就整筆放棄，導致翻唱眾多的
  熱門歌（`風になる`）永遠拿不到歌詞。現在只有長度差在 2.5 秒容差內、且雙方同步狀態
  相同時才視為真正無法區分。這是**刻意放寬安全性**以換取可用性，代價是可能選到別的
  演出版本——待實作的播放器內歌詞選擇按鈕就是對應的修正手段。
...
- **`愛唄` 這類極短標題**：寬鬆通道已能繞過短標題的歌手門檻，但這首歌在 LRCLIB 上翻唱
  眾多，仍可能觸發歧義否決而放棄。若實機仍查不到，需要一份 `[Pipeline]` log 判斷是門檻
  問題還是歧義否決。
