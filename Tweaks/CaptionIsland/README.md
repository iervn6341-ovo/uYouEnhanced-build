...
每一種歌名讀法都先走 LRCLIB `GET /api/search?q=`，讓歌名可以同時命中 LRCLIB 的歌曲、
歌手或專輯欄位；只有 `q=` 明確回傳 0 筆或 404，才用同一個歌名補查一次
`GET /api/search?track_name=`。流程永遠不再使用 `GET /api/get`。`/api/get` 把歌名、歌手、長度當成單一 AND 條件比對，而從上傳標題推導
...
主要原因。改以全文搜尋後，推導出的歌手仍只在本機作為排序訊號，不會成為 API 的 AND
條件；較寬的回應則繼續由歌名相似度與影片長度篩選。
...
**候選讀法清單**，依序搜尋，並由影片長度決定哪個猜對了。清單最多 6 個讀法
（`CISongQueryMaximumCandidates`），順序為：

| 順序 | 來源 | 說明 |
|---|---|---|
| 1 | `parsed`／明確書名框 | `CISplitSongMetadata` 選定的讀法；`「」`、`『』`、`《》` 內容屬於高可信標題來源 |
| 2 | `bilingual-bracket` | 同一個 `【】` 內的漢字與拉丁文字歌名分別成為候選 |
| 3 | `separator-right`／`separator-left` | `-`、`–`、`—`、`/`、`｜` 等分隔符的右、左側 |
| 4 | `colon-right`／`colon-left` | 冒號右、左側 |
| 5 | `no-feat` | 移除 `feat.`／`ft.` 之後的讀法 |
| 6 | `verbatim` | 完全不移除內容的原始標題兜底 |
...
封茗囧菌x雙笙 - 世末歌者「那怕只 一瞬的 奇蹟。」 [ High Quality Lyrics ][ Chinese Style ] tk推薦 益笙菌
  ...
  → separator-right  "世末歌者"              歌手提示 "封茗囧菌x雙笙"
  → separator-left   "封茗囧菌x雙笙"          歌手提示 "世末歌者"
```

`《》` 也會作為明確作品名稱的候選，但不會成為唯一讀法：例如 `《鬼滅之刃》主題曲 紅蓮華`
的括號可能是動畫而非歌曲，因此解析器仍會保留其他讀法交由 LRCLIB 結果與影片長度判斷。

`【】` 通常仍視為上傳裝飾；只有同一區塊同時包含一段漢字與一段拉丁文字，且沒有反覆切換
文字系統時，才會拆成第二順位候選。這不綁定任何特定歌曲，例如：

```
MC HotDog 熱狗 feat.張震嶽 A-Yue&關穎 Terri Kwan【嗨嗨人生 High High Life】Official Music Video
  → parsed              "MC HotDog 熱狗 feat.張震嶽 A-Yue&關穎 Terri Kwan"
  → bilingual-bracket  "嗨嗨人生"
  → bilingual-bracket  "High High Life"
```

單語的 `【Official Music Video】`、`【歌詞】` 等區塊仍只會被當成裝飾移除，不會浪費搜尋候選。
雙語名稱的兩種文字之間必須有空白或分隔符，因此 `【CC中日字幕】` 這類相連的字幕標籤不會
被拆成 `CC` 與 `中日字幕`。若標題採用 `作品資訊 插曲『歌名』完整版｜歌手`，管線右側會
優先作為歌手提示；例如 `【CC中日字幕】從零開始的異世界生活 第二季 插曲『Door』完整版｜高橋李依`
的第一個候選為 `Door — 高橋李依`，而不是把前面的作品說明誤認成歌手。
...
輸入的歌名是決定而不是猜測，會單獨搜尋，不會展開成多個讀法。自動播放與播放器內的
手動瀏覽都先使用 LRCLIB `q=` 全文模式搜尋歌名、歌手與專輯欄位；零結果時再以
`track_name=` 補查同一個歌名。

`CILRCLIBCacheQueryGeneration` 已提升為 7，因此舊版本記錄的「查無歌詞」會被丟棄
...
- **停用 `/api/get`**：所有查詢先走 `/api/search?q=`，零結果才補查
  `/api/search?track_name=`。舊路徑只要推導出
...
- **播放器內的 Caption Island 按鈕與快速面板**（`CIPlayerButton.xm`、
  ...
  面板目前提供三項，影片專屬的選擇都以 YouTube video ID 持久保存並在重看時自動套用：
  **字幕時間校正**（±3 秒／±0.5 秒／重設，沿用設定頁同一個 ±30 秒上限）與
  **YouTube 字幕來源挑選**（同語言的人工 `CC` 與自動 `ASR` 分開列出；點選後這支影片會
  優先使用該來源，不受全域「LRCLIB 優先」影響），以及 **LRCLIB 歌詞選擇器**。歌詞搜尋會
  先列出標題解析器產生的候選歌名，使用者選定其中一個才送出搜尋，也可自行輸入歌名；結果
  以 `q=` 同時搜尋 LRCLIB 的歌名、歌手與專輯欄位，避免本地化名稱只存在專輯欄位時漏掉；
  選單顯示的 `歌名 — 歌手` 會以 `歌名 歌手` 搜尋，破折號只負責畫面呈現、不會送進查詢；
  依影片長度排序並標示有無時間軸。手動列表以 LRCLIB 曲目本身的長度驗證同步時間軸，不會
  因 YouTube 影片長度不同而把「有時間軸」降級為「純文字」；長度差仍顯示供使用者判斷。
  選定後釘選給這支影片，亦可指定這支影片不使用歌詞。
...
- **一個標題送出多種讀法**：分隔符兩側、明確書名框，以及 `【中文 English】` 內分開的
  中英文名稱，都成為候選讀法，依序搜尋並由影片長度挑出最佳結果。上限 6 個讀法，全部經過同一個速率限制器；
...
- **`Precious Star Dreamer` 這類「無裝飾標題」的實機結果**：解析器對
  ...
  許範圍剔除候選。查詢會先使用 `q=` 全文搜尋，零結果再以 `track_name=` 補查，負向快取
  也會因查詢世代遞增而失效；若仍失敗，需要一份實機 log 確認是哪一道評分或長度門檻排除
  了回應。
